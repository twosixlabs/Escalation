...
jQuery.fn.dataTable.render.percentBar = function(pShape, cText, cBorder, cBar, cBack, vRound, bType) {
  ...
  return function(d, type, row) {
    ...
    //Round to the given parameter vRound
      s = 100 * parseFloat(d.toString().replace(/\s%|%/g,''))
      s = s.toFixed(0);
...
jQuery.fn.dataTable.render.scoreBar = function(pShape, cText, cBorder, cBar, cBack, vRound, bType) {
  pShape = pShape || 'square';
  cText = cText || '#000';
  cBorder = cBorder || '#BCBCBC';
    cBar = cBar || '#42c5f4';
  ...
  return function(d, type, row) {
    //Remove % if found in the value
    //Round to the given parameter vRound
      s = parseFloat(d.toString().replace(/\s%|%/g,''))
      s = s.toFixed(2);
    ...
    //Return the code for the bar
    return '<div style="'+styleRule1+'"><div style="'+styleRule2+'"><div style="'+styleRule3+'width:'+w+ '%;"></div><div style="width:100%;text-align:center;position:absolute;left:0;top:0;">'+s+'</div></div></div>';
  };
};
